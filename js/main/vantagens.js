  ...
  firebase.auth().onAuthStateChanged(function(user) {
    ...
    if (user) {
      ...
      statusRef.on("child_added", function(snapshot) {
        ...
        var vantId = vantNome;
        $('#myVantagens').append('<li id="'+childData.Nome+'"><button id="removeItem" type="submit" class="btn red-text" onClick="remove()"><i class="material-icons">add</i></button><div class="collapsible-header" id="vantNome">'+childData.Nome+'</div><div class="collapsible-body"><p id="vantCusto">'+childData.Custo+'</p><span id="vantDescr">'+childData.Descricao+'</span></div></li>');
      });
      ...
      var vantagensData = firebase.database().ref('Vantagens/');
          ...
          if ($(this).hasClass('active')) {
            ...
            setTimeout(function () {
              ...
              snapshot.forEach(function(childSnapshot){
                ...
                // console.log(childDataKey);
                $('#vantagemList .collapsible').append('<li id="'+childData.Nome+'"><div id="salvarVant"><input name="'+childData.Nome+'" type="checkbox" id="'+childData.Nome+'" /><label for="'+childData.Nome+'"></label></div><div class="collapsible-header" id="vantNome"><span class="badge">'+childData.Custo+'</span> <label>'+childData.Nome+'</label></div><div class="collapsible-body"><p id="vantCusto">'+childData.Custo+'</p><div class="divider"></div><span id="vantDescr">'+childData.Descricao+'</span></div></li>');
              })
              addVant(snapshot);
        ...
        if ($('#vantagemWrap').hasClass('scale-in')) {
            Materialize.showStaggeredList('#myVantagens>li');
        }
      });
    } else {
